...
#import "PSSPayPassViewController.h"
...
@interface PSSPayPassViewController ()

@end

@implementation PSSPayPassViewController
...
AC_SETUP_NAVIGATION_ITEM(NTVLocalized(@"修改支付密码"));
...
- (void)creatTableView {
    ...
    [row.cellConfig setObject:YXBColor_placeholder forKey:@"textField.placeholderColor"];
    [row.cellConfig setObject:YXBColor_titleText forKey:@"textField.textColor"];
    [row.cellConfig setObject:NTVLocalized(@"请输入确认密码") forKey:@"textField.placeholder"];
    [row.cellConfig setObject:@(YES) forKey:@"textField.secureTextEntry"];
    [section addFormRow:row];
...
-(void)DXCaptchaEventSuccess:(NSString *)token andBtn:(UIButton *)btn{
    ...
    MessageAPI *network = [[MessageAPI alloc] initWithPhone:userDescrip.value smsType:@"4" authToken:token];
...
- (void)confirmAction:(UIButton *)button {
    XLFormRowDescriptor *oldPassDescrip = [self.form formRowWithTag:@"1"];
    ...
    ModifyPayPassAPI *network = [[ModifyPayPassAPI alloc] initWithOldPass:oldPassDescrip.value newsPass:bankNumberDescrip.value code:addressDescrip.value];
    [network startWithCompletionBlockWithSuccess:^(__kindof ModifyPayPassAPI * _Nonnull request) {
        if ([request isValidRequestData]) {
            NSString *toastStr = NTVLocalized(@"修改成功");
            ShowToast(toastStr);
            POP;
        }
    } failure:^(__kindof YTKBaseRequest * _Nonnull request) {
