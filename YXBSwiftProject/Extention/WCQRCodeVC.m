...
- (void)setupNavigationBar {
    ...
    UIBarButtonItem *right = [[UIBarButtonItem alloc]initWithTitle:NSLocalizedString(@"相册",nil) style:UIBarButtonItemStyleDone target:self action:@selector(rightBarButtonItenAction)];
...
- (void)rightBarButtonItenAction {
    ...
    [obtain setBlockWithQRCodeObtainAlbumResult:^(SGQRCodeObtain *obtain, NSString *result) {
        if (result == nil) {
            NSLog(@"暂未识别出二维码");
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ShowToast(NSLocalizedString(@"没有发现二维码",nil))
            });
        } else {
