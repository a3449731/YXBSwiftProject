...
//                                          kCCKeySizeDES,
                                          ...
                                          &numBytesEncrypted);
    if (cryptStatus == kCCSuccess) {
        CocoaSecurityResult *result = [[CocoaSecurityResult alloc] initWithBytes:buffer length:numBytesEncrypted];
        free(buffer);
...
+ (CocoaSecurityResult *)aesECBDecryptWithData:(NSString *)dataString key:(NSString *)key {
    ...
    // 注意要这样转, 才能出结果
    NSData *data = [self dataForHexString:dataString];
    ...
    CCCryptorStatus cryptStatus = CCCrypt(kCCDecrypt,
                                          ...
                                          &numBytesCrypted);
    if (cryptStatus == kCCSuccess) {
        CocoaSecurityResult *result = [[CocoaSecurityResult alloc] initWithBytes:buffer length:numBytesCrypted];
        free(buffer);
