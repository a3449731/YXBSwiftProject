...
+ (void)renderGlobalAppearances {
    [self customMoreOperationAppearance];
    [self customAlertControllerAppearance];
    [self customDialogViewControllerAppearance];
    [self customImagePickerAppearance];
    [self customEmotionViewAppearance];
...
@implementation YXBUIHelper (QMUIMoreOperationAppearance)

+ (void)customMoreOperationAppearance {
    // 如果需要统一修改全局的 QMUIMoreOperationController 样式，在这里修改 appearance 的值即可
    [QMUIMoreOperationController appearance].cancelButtonTitleColor = [YXBThemeManager sharedInstance].tintColor;
}

@end


@implementation YXBUIHelper (QMUIAlertControllerAppearance)
...
@implementation YXBUIHelper (QMUIDialogViewControllerAppearance)
...
+ (void)customDialogViewControllerAppearance {
    ...
    appearance.backgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<YXBThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:YXBThemeIndetifierDark]) {
            return UIColorMake(34, 34, 34);
        }
        return UIColorWhite;
    }];
    appearance.headerViewBackgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<YXBThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:YXBThemeIndetifierDark]) {
            return UIColorMake(34, 34, 34);
        }
    ...
    appearance.headerSeparatorColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<YXBThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:YXBThemeIndetifierDark]) {
    ...
    buttonTitleAttributes[NSForegroundColorAttributeName] = [YXBThemeManager sharedInstance].tintColor;;
    appearance.buttonTitleAttributes = [buttonTitleAttributes copy];
    
    appearance.buttonHighlightedBackgroundColor = [[YXBThemeManager sharedInstance].tintColor colorWithAlphaComponent:.25];
}

@end


@implementation YXBUIHelper (QMUIEmotionView)
...
+ (void)customEmotionViewAppearance {
    ...
    [QMUIEmotionView appearance].sendButtonBackgroundColor = [YXBThemeManager sharedInstance].tintColor;;
}

@end

@implementation YXBUIHelper (QMUIImagePicker)
...
+ (void)customImagePickerAppearance {
    ...
    [QMUIImagePickerCollectionViewCell appearance].checkboxImage = [checkboxImage qmui_imageWithTintColor:[YXBThemeManager sharedInstance].tintColor];
    [QMUIImagePickerCollectionViewCell appearance].checkboxCheckedImage = [checkboxCheckedImage qmui_imageWithTintColor:[YXBThemeManager sharedInstance].tintColor];
    [QMUIImagePickerPreviewViewController appearance].toolBarTintColor = [YXBThemeManager sharedInstance].tintColor;
}

@end

@implementation YXBUIHelper (UITabBarItem)
...
@implementation YXBUIHelper (Button)
...
+ (QMUIButton *)generateDarkFilledButton {
    ...
    button.backgroundColor = [YXBThemeManager sharedInstance].tintColor;
    button.highlightedBackgroundColor = [[YXBThemeManager sharedInstance].tintColor qmui_transitionToColor:UIColorBlack progress:.15];// 高亮时的背景色
...
+ (QMUIButton *)generateLightBorderedButton {
    QMUIButton *button = [[QMUIButton alloc] qmui_initWithSize:CGSizeMake(200, 40)];
    button.titleLabel.font = UIFontBoldMake(14);
    button.tintColorAdjustsTitleAndImage = [YXBThemeManager sharedInstance].tintColor;
    button.backgroundColor = [[YXBThemeManager sharedInstance].tintColor qmui_transitionToColor:UIColorWhite progress:.9];
    button.highlightedBackgroundColor = [[YXBThemeManager sharedInstance].tintColor qmui_transitionToColor:UIColorWhite progress:.75];// 高亮时的背景色
    button.layer.borderColor = [button.backgroundColor qmui_transitionToColor:[YXBThemeManager sharedInstance].tintColor progress:.5].CGColor;
    button.layer.borderWidth = 1;
    button.layer.cornerRadius = 4;
    button.highlightedBorderColor = [button.backgroundColor qmui_transitionToColor:[YXBThemeManager sharedInstance].tintColor progress:.9];// 高亮时的边框颜色
    return button;
}
...
@implementation YXBUIHelper (Emotion)
...
+ (void)asyncLoadImages:(NSArray<QMUIEmotion *> *)emotions {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        for (QMUIEmotion *e in emotions) {
            e.image = [UIImageMake(e.identifier) qmui_imageWithBlendColor:[YXBThemeManager sharedInstance].tintColor];
        }
    });
}
...
@implementation YXBUIHelper (SavePhoto)
...
@implementation YXBUIHelper (Calculate)
...
@implementation YXBUIHelper (Theme)
