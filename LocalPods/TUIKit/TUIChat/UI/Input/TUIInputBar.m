...
- (void)setupViews
{
    ...
    [_giftButton setImage: [UIImage imageNamed:@"CU_XIAOXI_GIFT"]
    ...
    [_moreButton setImage:[UIImage imageNamed:@"CU_XIAOXI_IMAGE"]
                 forState:UIControlStateNormal];
    [_moreButton setImage:[UIImage imageNamed:@"CU_XIAOXI_IMAGE"]
                 forState:UIControlStateHighlighted];
    ...
    _inputTextView.backgroundColor = TUIChatDynamicColor(@"chat_input_bg_color", @"#F0F0F0");
    _inputTextView.textColor = TUIChatDynamicColor(@"chat_input_text_color", @"#333333");
    _inputTextView.layer.borderColor = [UIColor clearColor].CGColor;
    [_inputTextView setReturnKeyType:UIReturnKeySend];
    _inputTextView.textContainerInset = UIEdgeInsetsMake(10, 10, 0, 50);
    ...
    [_gameButton addTarget:self action:@selector(clickGameBtn:) forControlEvents:UIControlEventTouchUpInside];
    [_gameButton setImage: [UIImage imageNamed:@"CU_XIAOXI_GAME"]
                 forState:UIControlStateNormal];
    [_gameButton setImage: [UIImage imageNamed:@"CU_XIAOXI_GAME"]
                 forState:UIControlStateHighlighted];
    ...
    [_recordButton setTitleColor:TUIChatDynamicColor(@"chat_input_text_color", @"#333333") forState:UIControlStateNormal];
    _recordButton.backgroundColor = [UIColor colorWithHex:@"#F0F0F0"];
...
- (void)applyBorderTheme {
    if (_recordButton) {
        ...
        [_recordButton.layer setCornerRadius:TTextView_TextView_Height_Min/2];
        [_recordButton.layer setBorderWidth:0.0f];
        [_recordButton.layer setBorderColor:TUICoreDynamicColor(@"separator_color", @"#DBDBDB").CGColor];
    }
    
    if (_inputTextView) {
        [_inputTextView.layer setMasksToBounds:YES];
        [_inputTextView.layer setCornerRadius:TTextView_TextView_Height_Min/2];
        [_inputTextView.layer setBorderWidth:0.0f];
...
- (void)defaultLayout
{
    ...
    _giftButton.frame = CGRectMake(_moreButton.frame.origin.x - buttonSize.width - TTextView_Margin, buttonOriginY, buttonSize.width, buttonSize.height);
    
    _gameButton.frame = CGRectMake(_giftButton.frame.origin.x - buttonSize.width - TTextView_Margin, buttonOriginY, buttonSize.width, buttonSize.height);

    CGFloat beginX = _micButton.frame.origin.x + _micButton.frame.size.width + TTextView_Margin;
    CGFloat endX = _gameButton.frame.origin.x - TTextView_Margin;
    ...
    _faceButton.frame = CGRectMake(beginX+_inputTextView.bounds.size.width - buttonSize.width - TTextView_Margin, buttonOriginY, buttonSize.width, buttonSize.height);
}
...
- (void)layoutButton:(CGFloat)height
{
    ...
    CGRect gameFrame = _gameButton.frame;
    gameFrame.origin.y = originY;
    _gameButton.frame = gameFrame;
...
- (void)clickGameBtn:(UIButton *)sender {
    if(_delegate && [_delegate respondsToSelector:@selector(inputBarDidTouchGame:)]){
        [_delegate inputBarDidTouchGame:self];
    }
}
