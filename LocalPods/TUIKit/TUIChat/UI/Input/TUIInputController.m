...
- (void)keyboardWillShow:(NSNotification *)notification
{
    ...
    else if(_status == Input_Status_Input_Game){
        [self hideGameAnimation];
    }
    else{
...
//        ws.gameView.hidden = YES;
//        ws.gameView.alpha = 1.0;
//        [ws.gameView removeFromSuperview];
    }];
}
...
- (void)showGameAnimation
{
    ...
    self.gameView.hidden = NO;
    CGRect frame = self.gameView.frame;
    frame.origin.y = Screen_Height;
    self.gameView.frame = frame;
    __weak typeof(self) ws = self;
    [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        CGRect newFrame = ws.gameView.frame;
        newFrame.origin.y = ws.inputBar.frame.origin.y + ws.inputBar.frame.size.height;
        ws.gameView.frame = newFrame;
    } completion:nil];
}

- (void)hideGameAnimation
{
    self.gameView.hidden = NO;
    self.gameView.alpha = 1.0;
    __weak typeof(self) ws = self;
...
- (void)inputBarDidTouchMore:(TUIInputBar *)textView
{
    ...
    if(_status == Input_Status_Input_Game){
        [self hideGameAnimation];
    }
    [_inputBar.inputTextView resignFirstResponder];
...
- (void)inputBarDidTouchGame:(TUIInputBar *)textView {
    if(_status == Input_Status_Input_Game){
        return;
    }
    if(_status == Input_Status_Input_Face){
    ...
    [_inputBar.inputTextView resignFirstResponder];
    [self showGameAnimation];
    _status = Input_Status_Input_Game;
    if (_delegate && [_delegate respondsToSelector:@selector(inputController:didChangeHeight:)]){
        [_delegate inputController:self didChangeHeight:CGRectGetMaxY(_inputBar.frame) + self.moreView.frame.size.height + Bottom_SafeHeight];
    }
}
...
- (UIView *)gameView {
    if (!_gameView) {
        _gameView = [[UIView alloc] initWithFrame:CGRectMake(0, _inputBar.frame.origin.y + _inputBar.frame.size.height, self.view.frame.size.width, TFaceView_Height + 30)];
        NSArray *imageArr = @[@"CUHC_game_组 2514", @"CUHC_game_组 2517"];
        for (NSInteger i = 0; i < imageArr.count; i ++) {
            UIButton *optionBtn = [[UIButton alloc] init];
            optionBtn.frame = CGRectMake(15 + (100+10) * i, 15, 100, 70);
            [optionBtn addTarget:self action:@selector(clickGameItemButtonClick:) forControlEvents:UIControlEventTouchUpInside];
            [optionBtn setBackgroundImage:[UIImage imageNamed:imageArr[i]] forState:UIControlStateNormal];
            [optionBtn setBackgroundImage:[UIImage imageNamed:imageArr[i]]
                         forState:UIControlStateHighlighted];
            optionBtn.tag = 9980+i;
            [_gameView addSubview:optionBtn];
        }
    }
    return _gameView;
}

- (void)clickGameItemButtonClick:(UIButton *)sender {
    if (sender.tag == 9980) {
        if(_delegate && [_delegate respondsToSelector:@selector(inputControllerGobangClick:)]){
            [_delegate inputControllerGobangClick:self];
        }
    } else {
        if(_delegate && [_delegate respondsToSelector:@selector(inputControllerCartagenaClick:)]){
            [_delegate inputControllerCartagenaClick:self];
        }
    }
}
