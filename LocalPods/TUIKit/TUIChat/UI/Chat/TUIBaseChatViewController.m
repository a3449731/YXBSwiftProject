...
- (void)setupMessageController {
    ...
    self.biaoQingView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 125)];
    self.biaoQingView.hidden = YES;
    [self.view addSubview:self.biaoQingView];
...
- (void)inputControllerCartagenaClick:(TUIInputController *)inputController {
    !self.optionBlock ? : self.optionBlock(2);
}

- (void)inputControllerGobangClick:(TUIInputController *)inputController {
    !self.optionBlock ? : self.optionBlock(1);
}
...
- (void)inputController:(TUIInputController *)inputController didChangeHeight:(CGFloat)height
{
    ...
    [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (self.haveDazhaohu) {
            msgFrame.size.height = self.view.frame.size.height - height - [self topMarginByCustomView]-125;
            self.messageController.view.frame = msgFrame;
            
            CGRect biaoqingFrame = self.biaoQingView.frame;
            biaoqingFrame.origin.y = msgFrame.size.height;
            self.biaoQingView.frame = biaoqingFrame;
...
- (void)setHaveDazhaohu:(BOOL)haveDazhaohu
{
    if (haveDazhaohu) {
        self.biaoQingView.hidden = NO;
//        CGRect mesFrame = self.messageController.view.frame;
//        mesFrame.size.height = mesFrame.size.height-125;
//        self.messageController.view.frame = mesFrame;
    } else {
        if (_haveDazhaohu) {
...
- (void)inputController:(TUIInputController *)inputController didSendMessage:(V2TIMMessage *)msg
{
    if ([self.delegate respondsToSelector:@selector(didSendMessage:)]) {
        [self.delegate didSendMessage:msg];
    }
