...
@interface ReplayViewController ()<ReplayControlViewDelegate, ReplayDelegate>

@property (weak, nonatomic) IBOutlet UIView *whiteboardBaseView;
@property (weak, nonatomic) IBOutlet UIView *controlBgView;
@property (weak, nonatomic) ReplayControlView *controlView;
...
// replay
@property (nonatomic, strong) ReplayManager *replayManager;
@property (nonatomic, weak) UIView *boardView;
@property (nonatomic, weak) ReplayVideoView *videoView;
...
- (void)initData {
    ...
    self.replayManager = [ReplayManager new];
...
- (void)setupView {
    ...
    NSBundle *baseBundle = [NSBundle bundleForClass:ReplayViewController.class];

    ReplayControlView *controlView = [baseBundle loadNibNamed:@"ReplayControlView" owner:self options:nil].firstObject;
    [self.controlBgView addSubview:controlView];
    [self equalFrom:controlView to:self.controlBgView];
    self.controlView = controlView;

    LoadingView *loadingView = [baseBundle loadNibNamed:@"LoadingView" owner:self options:nil].firstObject;
    [self.loadingBgView addSubview:loadingView];
    [self equalFrom:loadingView to:self.loadingBgView];
    self.loadingView = loadingView;

    ReplayVideoView *videoView = [[ReplayVideoView alloc] initWithFrame:CGRectZero];
