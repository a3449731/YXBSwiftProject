...
@implementation VideoReplayService
...
- (void)initWithConfiguration:(ReplayConfiguration *)config success:(void (^) (AVPlayer *avPlayer))successBlock failure:(void (^) (NSError * error))failBlock {
    ...
    NSURL *url = [NSURL URLWithString:config.videoConfig.urlString];
    _nativePlayer = [AVPlayer playerWithURL:url];
    _playbackSpeed = 1;
    [self registerNotificationAndKVO];
    if(successBlock) {
        successBlock(self.nativePlayer);
    }
}
...
- (void)playerItemDidReachEnd:(NSNotification *)notification {
    if (notification.object == self.nativePlayer.currentItem && [self.delegate respondsToSelector:@selector(videoReplayDidFinish)]) {
...
- (void)nativeStartBuffering {
    if ([self.delegate respondsToSelector:@selector(videoReplayStartBuffering)]) {
        [self.delegate videoReplayStartBuffering];
    }
}

- (void)nativeEndBuffering {
   if ([self.delegate respondsToSelector:@selector(videoReplayEndBuffering)]) {
       [self.delegate videoReplayEndBuffering];
   }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object
    ...
    if (object == self.nativePlayer && [keyPath isEqualToString:kCurrentItemKey]) {
        ...
        if ([self.delegate respondsToSelector:@selector(videoReplayStateChange:)]) {
            [self.delegate videoReplayStateChange:[self videoDesireToPlay]];
        }
    } else if ([keyPath isEqualToString:kStatusKey]) {
        
        if (self.nativePlayer.status == AVPlayerStatusFailed) {
            [self pause];
            if ([self.delegate respondsToSelector:@selector(videoReplayError:)]) {
                [self.delegate videoReplayError:self.nativePlayer.error];
            }
        }
    } else if ([keyPath isEqualToString:kPlaybackLikelyToKeepUpKey]) {

        NSString *logString = [NSString stringWithFormat:@"isPlaybackLikelyToKeepUp %d", self.nativePlayer.currentItem.isPlaybackLikelyToKeepUp];
        [AgoraLogManager logMessage:logString level:AgoraLogLevelInfo];
        ...
        NSString *logString = [NSString stringWithFormat:@"isPlaybackBufferFull %d", self.nativePlayer.currentItem.isPlaybackBufferFull];
        [AgoraLogManager logMessage:logString level:AgoraLogLevelInfo];
        ...
        NSString *logString = [NSString stringWithFormat:@"isPlaybackBufferEmpty %d", self.nativePlayer.currentItem.isPlaybackBufferEmpty];
        [AgoraLogManager logMessage:logString level:AgoraLogLevelInfo];
        ...
        if ([self.delegate respondsToSelector:@selector(videoReplayLoadedTimeRangeChange:)]) {
            [self.delegate videoReplayLoadedTimeRangeChange:timeRanges];
        }
    }
}
